...
bg_css = f"background: url('{bg_url}') no-repeat center center fixed; background-size: cover;" if bg_url else ""
st.markdown(
    """
    <style>
    .stApp {
...
def clean_invisibles(s: str) -> str:
    """Remove invisíveis (BOM/zero-width/NBSP) sem mexer nas quebras."""
    bad = ["\ufeff", "\u200b", "\u200c", "\u200d", "\u2060", "\xa0"]
    for ch in bad:
...
# ===================== TABS =====================
tab_gen, tab_forum = st.tabs(["🧩 Gerar plano", "🗣️ Fórum (debate e melhorias)"])

# ------------------------------------------------
# TAB: GERAR PLANO (mantém seu fluxo estável)
# ------------------------------------------------
with tab_gen:
    # ---------- Ajuda + botões ----------
    st.markdown(
...
# ------------------------------------------------
# TAB: FÓRUM (debate e melhorias) — simples com SQLite
# ------------------------------------------------
with tab_forum:
    ...
    if not ENABLE_FORUM:
        ...
        # Subtabs por categoria
        cat_tabs = st.tabs(["📢 Atualizações", "💡 Sugestões", "🧨 Críticas"])
        ...
        for cat_tab, CAT in zip(cat_tabs, categories):
            with cat_tab:
                ...
                # --- FORM DE NOVO POST (somente admin) ---
                if is_admin:
                    ...
                    if posted:
                        if not title.strip() or not body_md.strip():
                            ...
                            # Salvar imagens (sem apagar no futuro)
                            saved_paths = []
                ...
                if not rows:
                    ...
                    for pid, ts_post, title, body_md, images_json in rows:
                        ...
                        with st.container(border=True):
                            st.write(f"### {title}")
                            st.caption(dt)
                            st.markdown(body_md)  # Markdown do admin
                            ...
                            if COMMENTS_ENABLED:
                                ...
                                if send:
                                    # anti-flood simples: 30s por UID
